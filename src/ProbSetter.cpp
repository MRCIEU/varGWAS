...
ProbSetter::ProbSetter(Data *result) :
    m_result(result),
    m_sample_i(0) {}

// Called once allowing us to set storage.
void ProbSetter::initialise(std::size_t number_of_samples, std::size_t number_of_alleles) {
  m_result->clear();
...
void ProbSetter::set_min_max_ploidy(uint32_t min_ploidy, uint32_t max_ploidy, uint32_t min_entries,
...
bool ProbSetter::set_sample(std::size_t i) {
  m_sample_i = i;
...
void ProbSetter::set_number_of_entries(std::size_t ploidy, std::size_t number_of_entries,
...
void ProbSetter::set_value(uint32_t, double value) {
  m_result->at(m_sample_i).at(m_entry_i++) = value;
};

// Ditto, but called if data is missing for this sample.
void ProbSetter::set_value(uint32_t, genfile::MissingValue value) {
