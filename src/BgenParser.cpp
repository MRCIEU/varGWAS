...
BgenParser::BgenParser(std::string const &filename) : m_filename(filename), m_state(e_NotOpen),
  ...
  read_offset(*m_stream, &m_offset);
  read_header_block(*m_stream, &m_context);
  if (m_context.flags & e_SampleIdentifiers) {
    read_sample_identifier_block(
        *m_stream, m_context,
...
std::ostream &BgenParser::summarise(std::ostream &o) const {
  o << "BgenParser: bgen file ("
    << (m_context.flags & e_Layout2 ? "v1.2 layout" : "v1.1 layout")
    << ", "
    << (m_context.flags & e_CompressedSNPBlocks ? "compressed" : "uncompressed") << ")"
    << " with "
...
std::size_t BgenParser::number_of_samples() const {
...
void BgenParser::read_probs(std::vector<std::vector<double> > *probs) {
  ...
  read_and_parse_genotype_data_block<ProbSetter>(
      *m_stream,
      m_context,
      setter,
      &m_buffer1,
...
void BgenParser::ignore_probs() {
  ignore_genotype_data_block(*m_stream, m_context);
  m_state = e_ReadyForVariant;
};
