...
for (phi in seq(0, 6, 0.5)) {
  ...
  for (lambda in c(1, 10, 100, 1000)) {
    for (i in 1:n_sim) {
      ...
      # run vGWAS
      system("../build/bin/jlst_cpp -v data/phenotypes.csv -s , -o data/gwas-bp.txt -b data/genotypes.bgen -p Y -i S -t 1")
      system("../build/bin/jlst_cpp -v data/phenotypes.csv -s , -o data/gwas-bf.txt -b data/genotypes.bgen -p Y -i S -t 1 -r")
